...
{
	unique_ptr<BattleshipCompetitionManager> manager;
	...
	if (argc > i)
	{
		...
		try
		{
			...
			manager = make_unique<BattleshipCompetitionManager>(numThreads);
		}
		...
		manager = make_unique<BattleshipCompetitionManager>();
