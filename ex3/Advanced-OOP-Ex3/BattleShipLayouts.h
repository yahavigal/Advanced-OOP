...
class BattleShipNavy
{
	...
	static bool BattleShipNavy::isContinuousShip(char*** board, const Coordinate& p, char m, int model_size, BattleShipPiece::ShipOrientation orientation);
...
public:
	...
	//BattleShipNavy(const BattleShipNavy & navy)
	//{
	//	cout << "copying navy" << endl;
	//	rows_ = navy.rows_;
	//	cols_ = navy.cols_;
	//	depth_ = navy.depth_;
	//	aShips = navy.aShips;
	...
	//	for (auto ship : navy.ships_)
	//		ships_.emplace_back(new BattleShipPiece(*ship));
	//}
	...
	*/
	...
	int isValidShipAt(char*** board, const Coordinate& p, char m);
...
class BattleshipBoard : public BoardData
{
	...
	void allocate()
	{
		mBoard = new char**[_rows];
		for (int i = 0; i < _rows; ++i)
		{
			mBoard[i] = new char*[_cols];
			for (int j = 0; j < _cols; ++j)
				mBoard[i][j] = new char[_depth];
		}
	}
