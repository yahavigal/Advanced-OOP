...
bool BattleShipNavy::isContinuousShip(Arr_3D board, const Coordinate& p, char m, int model_size, BattleShipPiece::ShipOrientation orientation)
{
	int x, y;

	if (orientation == BattleShipPiece::ShipOrientation::Horizontal) {
		for (x = p.col + 1; x < p.col + model_size; x++) {
		...
		for (y = p.row + 1; y < p.row + model_size; y++) {
			if (board[y][p.col][p.depth] != m)
				return false;
		}
	}
	else {
		for (y = p.depth + 1; y < p.depth + model_size; y++) {
...
bool BattleShipNavy::isOverlappingItself(Arr_3D board, const Coordinate& p, char m, int model_size, BattleShipPiece::ShipOrientation orientation)
{
	...
	if (orientation == BattleShipPiece::ShipOrientation::Horizontal) {
		for (x = p.col; x < p.col + model_size; x++)
		{
	...
	if (orientation == BattleShipPiece::ShipOrientation::Vertical) {
		for (y = p.row; y < p.row + model_size; y++)
		{
	...
	if (orientation == BattleShipPiece::ShipOrientation::Depthical) {
		for (y = p.depth; y < p.depth + model_size; y++)
		{
...
int BattleShipNavy::isValidShipAt(Arr_3D board, const Coordinate& p, char m)
{
	...
	// Check for vertical ship
		...
		//	 "Wrong size or shape for ship " << m << " for player " << (player ? "B" : "A") << endl;
...
{
	int bound = min(static_cast<int>(line.length()), cols());
					...
					if (m == BattleShipModel::fromChar(mBoard[x][y][z]))
					{
						...
						if (valid != -1)
						{
							if (mBoard[x][y][z] == tolower(mBoard[x][y][z]))
								mNavy.addBattleShip(make_shared<BattleShipPiece>(Coordinate(x, y, z), m, static_cast<BattleShipPiece::ShipOrientation>(valid), 1));
							else mNavy.addBattleShip(make_shared<BattleShipPiece>(Coordinate(x, y, z), m, static_cast<BattleShipPiece::ShipOrientation>(valid), 0));
						}
					}
				}
			}
